...
public class Dungeon {
	...
	public static void startRun() {
		if (startTime == -1) {
			startTime = Game.realTime;
			if (SPDSettings.numberOfRuns(category) > 0) {
				goalSplitTime += ((double)SPDSettings.personalBest(category) / (double)SPDSettings.averageRunTime(category)) * SPDSettings.averageSplitTime(category, 1);
			}
//			Dungeon.level.seal();
		}
	}
	...
	public static long lastSplitTime;
	public static long goalSplitTime;
	...
	//todo 修复神必的自定义楼层csv问题，为什么会这样呢？摆烂.JPG
	...
	private static final String LAST_SPLIT_TIME= "last_split_time";
	...
	public static void saveGame( int save ) {
		try {
			...
			bundle.put( START_TIME, startTime );
			bundle.put( CURRENT_TIME, currentTime );
			bundle.put( LAST_SPLIT_TIME, lastSplitTime );
			bundle.put( FINISH_TIME, finishTime );
			bundle.put( CATEGORY, category );
			bundle.put( SPLITS, splits );
	...
	public static void loadGame( int save, boolean fullLoad ) throws IOException {
		...
		currentTime = bundle.getLong( CURRENT_TIME );
		lastSplitTime = bundle.getLong( LAST_SPLIT_TIME );
		finishTime = bundle.getLong( FINISH_TIME );
		category = bundle.getString( CATEGORY );
		splits = bundle.getLongArray( SPLITS );
